...
function recordedRequests(): string[] {
  ...
  const records = JSON.parse(readFileSync(file, 'utf-8')) as { method: string; url: string }[];
  return records.map(record => `${record.method} ${record.url}`);
}
...
describe('SentryCliAdapter against the real CLI', () => {
  it('creates a release', async () => {
    ...
    await createAdapter().createRelease('1.0.0');

    expect(requestsSince(before)).toContain('POST /api/0/organizations/test-org/releases/');
  });
