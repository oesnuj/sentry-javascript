...
- The HTTP span attributes `http.method`, `http.status_code`, `http.scheme`, `http.user_agent`, `http.flavor`, `http.client_ip`, `http.response_content_length` and `http.response_transfer_size` were renamed to `http.request.method`, `http.response.status_code`, `url.scheme`, `user_agent.original`, `network.protocol.version`, `client.address`, `http.response.body.size` and `http.response.size`. `http.request_content_length`, `http.request_content_length_uncompressed`, `http.response_content_length_uncompressed` and `http.status_text` are not part of `@sentry/conventions` and keep their names.
- The `http.target` span attribute is no longer set. Its pathname half is on `url.path` and its query half on `url.query` — server spans now set `url.query` and `url.fragment`, which previously only the `@sentry/node` server spans did. If you match on `http.target` in `ignoreSpans`, `beforeSendSpan`, dashboards or alerts, match on `url.path` (and `url.query` for query matches) instead.
- `network.*` span attributes were aligned across SDKs. On HTTP spans the `http.host` and `net.host.name` attributes were replaced by `server.address` and `net.host.port` by `server.port`. On HTTP **server** spans the socket peer is the client, so its address and port are reported on `network.peer.address`/`network.peer.port` and the local socket on `network.local.address`/`network.local.port`; `server.address`/`server.port` come from the `Host` header, which no longer carries the port inline. The raw header remains available on `http.request.header.host`.
- The `net.peer.ip` and `net.host.ip` span attributes on HTTP spans were renamed to `network.peer.address` and `network.local.address`.
- The `url.same_origin` span attribute on browser resource spans was renamed to `http.request.same_origin`.
